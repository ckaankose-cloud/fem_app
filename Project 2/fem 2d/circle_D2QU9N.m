...
for i=1:m*(1+inclusion)
    for j=1:4*p    
            if j==4*p
                EL((i-1)*(4*p)+j,1) = (2*i*8*p)-16*p+2*j-1;  
                EL((i-1)*(4*p)+j,2) = (2*i*8*p)-16*p+2*j+1 - 8*p;
                EL((i-1)*(4*p)+j,3) = (2*i*8*p)+2*j+1 - 8*p;
                EL((i-1)*(4*p)+j,6) = (2*i*8*p)-8*p+2*j+1 - 8*p;     
            else
                EL((i-1)*(4*p)+j,1) = (2*i*8*p)-16*p+2*j-1;  
                EL((i-1)*(4*p)+j,2) = (2*i*8*p)-16*p+2*j+1;
                EL((i-1)*(4*p)+j,3) = (2*i*8*p)+2*j+1;
                EL((i-1)*(4*p)+j,6) = (2*i*8*p)-8*p+2*j+1;
            end
            
            EL((i-1)*(4*p)+j,4) = (2*i*8*p)+2*j-1;
            EL((i-1)*(4*p)+j,5) = (2*i*8*p)-16*p+2*j;
            EL((i-1)*(4*p)+j,7) = (2*i*8*p)+2*j;
            EL((i-1)*(4*p)+j,8) = (2*i*8*p)-8*p+2*j-1;
            EL((i-1)*(4*p)+j,9) = (2*i)*(8*p)-8*p+2*j;
    end   
end 
...
%                 El_sqr(j,6) = 2*4*p-2;
%                 El_sqr(j,7) = ;
%                 El_sqr(j,8) = 2*4*p-j+1;
%                 El_sqr(j,9) = 2*4*p-j+2;
%             elseif j==p
%                 El_sqr(j,1) = j;
%                 El_sqr(j,2) = j+1;
%                 El_sqr(j,3) = j+2;
...
%         end
%     
%     El_sqr = El_sqr + size(EL,1) - p^2;
%     EL(NoE-p^2+1:NoE,:) = El_sqr(:,:);
% end

%%%%%%%%%%% WRITING INCLUSION'S ELEMENTS NOT SUCCESFULL%%%%%%%%%%%%%%%
% if inclusion
% %%%%%%%%%%%%%% elements inside mid square %%%%%%%%%%%%%%%
%     El_sqr = zeros((p)^2,NPE);
% for i=1:p
%     for j=1:p
%         El_sqr((i-1)*(p)+j,1) = 2*(i-1)*(p*2+1)+(j-1)*2+1;    
%         El_sqr((i-1)*(p)+j,2) = El_sqr((i-1)*(p)+j,1)+2;
%         El_sqr((i-1)*(p)+j,4) = El_sqr((i-1)*(p)+j,1)+(4*p-1)*2;
%         El_sqr((i-1)*(p)+j,3) = El_sqr((i-1)*(p)+j,4)+(2*p+1)*2;
%         El_sqr((i-1)*(p)+j,5) = El_sqr((i-1)*(p)+j,1)+1;
%         El_sqr((i-1)*(p)+j,6) = El_sqr((i-1)*(p)+j,2)+(4*p-1);
%         El_sqr((i-1)*(p)+j,7) = El_sqr((i-1)*(p)+j,3)-1;
%         El_sqr((i-1)*(p)+j,8) = El_sqr((i-1)*(p)+j,1)+(2*p+1)*3;
%         El_sqr((i-1)*(p)+j,9) = El_sqr((i-1)*(p)+j,8)+1;
%     end
% end
% 2*(p+1)*(2*m+1) + 2*(p-1)*(2*m+1) - p*4 + 1;
% 
%             elseif j==p
%                 El_sqr(j,1) = j;
%                 El_sqr(j,2) = j+1;
...
%         for j = 1:p
%             if (i == 1)&&(j == 1)
%                 EL((i-1)*p+j+num_nodes,1) = 2*(i-1)*(p*2+1)+(j-1)*2+1;
%                 EL((i-1)*p+j+num_nodes,2) = num_nodes_left+2;
