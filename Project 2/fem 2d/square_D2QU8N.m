function [NL, EL] = square_D2QU8N(d1, d2, p, m, e, inclusion)
...
%%%% Region 1 - Void %%%%           %%%% Region 1 bottom %%% from left to right
...
for i=1:m
    for j=1:4*p    
            ...
            if j==4*p %%%to prevent matrix index overflow
...
%%% Makes middle nodes infinity
for i=1:m
    for j=1:4*p
        NL(8*p*(2*i-1)+2*j,:)=[Inf Inf];
    end
end

%%% If coordinates of the node are infinty it omits it and creates correct NL for 8 Noded Element Type
NoN_8N = NoN-NoE; %we have NoE number of middle nodes
NL_8N = zeros(NoN_8N,PD);
count=1;
for i=1:NoN
    if(NL(i,:) ~= [Inf Inf])
        NL_8N(count,:)=NL(i,:);
...
for i=1:m
    for j=1:4*p
        if (j==4*p)%% careful end nodes create problems
            EL((i-1)*(4*p)+j,1) = (i-1)*12*p+2*j-1;
            EL((i-1)*(4*p)+j,2) = (i-1)*12*p+2*j+1 - 8*p;
            EL((i-1)*(4*p)+j,3) = (i)*12*p+2*j+1 - 8*p;
            EL((i-1)*(4*p)+j,6) = (i)*(8*p)+(4*p)*(i-1)+1+j - 4*p; 
        else
            EL((i-1)*(4*p)+j,1) = (i-1)*12*p+2*j-1;
            EL((i-1)*(4*p)+j,2) = (i-1)*12*p+2*j+1;
            EL((i-1)*(4*p)+j,3) = (i)*12*p+2*j+1;
            EL((i-1)*(4*p)+j,6) = (i)*(8*p)+(4*p)*(i-1)+1+j; 
        end
            EL((i-1)*(4*p)+j,1) = (i-1)*12*p+2*j-1;
            EL((i-1)*(4*p)+j,4) = (i)*12*p+2*j-1;
            EL((i-1)*(4*p)+j,5) = (i-1)*12*p+2*j;
            EL((i-1)*(4*p)+j,7) = (i)*12*p+2*j;
            EL((i-1)*(4*p)+j,8) = (i)*(8*p)+(4*p)*(i-1)+j;
    end
end
